...
Definition queue_eval_op (o : Queue.Op) (vs : list Value) : list Value :=
  match o, vs with
  | Empty, _ => [Q empty]
...
#[global] Instance queue_pure_impl : PureImpl Queue.Op Value := queue_eval_op.
...
Definition queue_exec_op (o : Queue.Op) (vs : list Value) : M (list Value) :=
  match o, vs with
  ...
  | New x, _ => ret [E (Core.Thunk x)]
  | _, _ => ret []
  end.

#[global] Instance queue_cv_impl : CvImpl Queue.Op Value := queue_exec_op.
